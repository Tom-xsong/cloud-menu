...
Page({
 ...
 async onShow() {
  this.data.page = 0
  let result1 =await db.collection("menu").orderBy("views","desc").limit(5).get()
  console.log(result1)
    this.setData({
      list:result1.data
    })
  ...
  async getList(page, pageSize) {
    ...
    var result = await db.collection("menu").orderBy("views","desc").skip(page * pageSize).limit(this.data.pageSize).get()
    console.log(result)
